...
def concatenate(tensors, axis=-1):
    ...
    return tf.concat(tensors, axis)          # modern tf.concat order
...
def load_weights():
    ...
    for name in WEIGHTS:
        if 'conv' in name:
            ...
            # This transpose converts [out, in, H, W] → [H, W, in, out] (correct for channels_last)
...
def img_to_encoding(image_path, model):
    ...
    # ====================== CHANGED ======================
    # For channels_last we keep (H, W, C) — do NOT transpose
    img = np.around(img / 255.0, decimals=12)
    # =====================================================
    
    x_train = np.expand_dims(img, axis=0)          # shape: (1, 96, 96, 3)
